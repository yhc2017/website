...
$c = substr($a,3);
$sql=mysqli_query($conn,"select * from article where a_id='".$c."';");
    ...
    <div class="body-content">
        <form class="form-x">
            <div class="form-group">
                ...
                <div class="field">
                    <input type="text" id="titleid" class="input w51" value="<?php echo $info['a_title']?>" name="title" data-validate="required:标题不能为空！请输入标题" />

<!--                    <input type="hidden" id="showdetail">-->

                    <div class="tips"></div>
                    <div id="showdetail">
<!--                        --><?php //echo file_get_contents("../public/article/267.txt");?>
                    </div>
            ...
            <div class="field" style="text-align: center;">
                ...
                <button type="button" id="submit" class="button bg-main icon-check-square-o"> 修改</button>
            </div>
        </form>
    </div>
</div>
...
$(document).ready(function(){
    ...
    var detailUrl = "../public/article/<?php echo $info['a_id']?>.txt";

    ue.ready(function(){

        <?php
            $text = htmlentities(file_get_contents("../public/article/{$info['a_id']}.txt"),ENT_QUOTES,"utf-8");
            echo "var text = '".html_entity_decode($text)."';";
        ?>
...
<script>
    ...
    btn.addEventListener("click",function(){
        ...
        var ll = <?php echo $info['a_id']?>;

        var se=confirm("是否确认修改?");
        if (se==true) {

            if(kk != "") {
                $.post("article_alter_help.php",{content:jj,title:kk,aid:ll},function(data){
                    if(data != "") {
                        alert("修改成功！");
